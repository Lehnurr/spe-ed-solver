...
public class PlaySimulationCommand implements Runnable {
	...
	public void setBoardWidth(final int boardWidth) {
		if (boardWidth <= 0) {
			ApplicationLogger
					.logAndThrowException(new ParameterException(spec.commandLine(), "The board width must be > 0!"));
		}
	...
	public void setBoardHeight(final int boardHeight) {
		if (boardHeight <= 0) {
			ApplicationLogger
					.logAndThrowException(new ParameterException(spec.commandLine(), "The board height must be > 0!"));
		}
	...
	public void setPlayerCount(final int playerCount) {
		if (playerCount < 2) {
			ApplicationLogger.logAndThrowException(
					new ParameterException(spec.commandLine(), "You need at least 2 players to simulate a game!"));
		}
		if (playerCount > 6) {
			ApplicationLogger.logAndThrowException(
					new ParameterException(spec.commandLine(), "You can't play spe_ed with more than 6 players!"));
		}
